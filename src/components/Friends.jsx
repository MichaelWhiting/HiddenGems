...
function Friends() {
    ...
    const friendCards = currentFriends.map((friend, i) => (
        <div key={i} className="friend-card" style={{ animation: animStr(i) }} onClick={() => navigateToFriendProfile(friend.userId)}>
            {friend?.imgUrl && (
                <img src={friend.imgUrl} alt={friend.firstName} className="friend-image" />
            )}
